// Adapted from Overleaf source-editor/hunspell/hunspell.worker.ts
// Removed Overleaf-specific imports (metrics, logging, error-reporter)
// Changed WASM import to use Vite's ?url suffix for correct URL resolution

import Hunspell from './wasm/hunspell.mjs'
...
const createSpellChecker = async ({
  ...
  const [dic, aff] = await Promise.all([
    fetch(new URL(`./${lang}.dic`, dictionariesRootURL)).then(response => {
      if (!response.ok) throw new Error(`Failed to load dictionary: ${response.url} (${response.status})`)
      return response.arrayBuffer()
    }),
    fetch(new URL(`./${lang}.aff`, dictionariesRootURL)).then(response => {
      if (!response.ok) throw new Error(`Failed to load affix file: ${response.url} (${response.status})`)
      return response.arrayBuffer()
    }),
  ])
